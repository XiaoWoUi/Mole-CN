...
func (m model) View() string {
	...
	if m.deleting {
		...
		if count > 0 {
			fmt.Fprintf(&b, "%s%s%s%s 正在删除: %s%s 项%s 已移除,请稍候...\n",
				colorCyan, colorBold,
				spinnerFrames[m.spinner],
				colorReset,
				colorYellow, formatNumber(count), colorReset)
		} else {
			fmt.Fprintf(&b, "%s%s%s%s 正在删除:正在移入废纸篓,请稍候...\n",
				colorCyan, colorBold,
				spinnerFrames[m.spinner],
				colorReset)
		}
	...
	if m.inOverviewMode() {
		...
		if selectCount > 0 {
			if largeFileCount > 0 {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格选择 | 回车 | / 筛选 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 %d | T 大文件 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, selectCount, largeFileCount, colorReset)
			} else {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格选择 | 回车 | / 筛选 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, selectCount, colorReset)
			}
		} else {
			if largeFileCount > 0 {
				fmt.Fprintf(&b, "%s↑↓←→ | 空格选择 | 回车 | / 筛选 | R 刷新 | O 打开 | P 预览 | F 文件 | ⌫ 删除 | T 大文件 %d | Esc 返回 | Q/Ctrl+C 退出%s\n", colorGray, largeFileCount, colorReset)
			} else {
