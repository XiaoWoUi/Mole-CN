...
func TestCalculateHealthScoreDetectsIssues(t *testing.T) {
	...
	if !strings.Contains(msg, "CPU 占用过高") {
		t.Fatalf("message 应提及 CPU 问题: %q", msg)
	}
...
func TestCalculateHealthScoreCapsFailingSMARTAt44(t *testing.T) {
	...
	if !strings.Contains(msg, "磁盘 SMART 异常") {
...
func TestBatteryHealthLabel(t *testing.T) {
	tests := []struct {
		...
		{"high cycles", 950, 85, "尽快维护", "danger"},
		{"low capacity", 200, 55, "尽快维护", "danger"},
