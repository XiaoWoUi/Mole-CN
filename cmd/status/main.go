...
var (
	// Command-line flags
	jsonOutput       = flag.Bool("json", false, "输出 JSON 格式指标而非 TUI 界面")
	procCPUThreshold = flag.Float64("proc-cpu-threshold", 100, "当某进程持续超过该 CPU 百分比时告警")
	procCPUWindow    = flag.Duration("proc-cpu-window", 5*time.Minute, "进程必须持续超过 CPU 阈值的时长")
	...
	watchMode     = flag.Bool("watch", false, "以换行分隔的 JSON 持续输出指标,代替一次性 TUI/JSON")
	watchInterval = flag.String("interval", "", "与 --watch 搭配的采集间隔(如 1s、2s);默认 1s")
)
...
func runJSONMode() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "收集指标出错: %v\n", err)
		os.Exit(1)
	}
...
func parseWatchInterval(raw string) (time.Duration, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("--interval %q 无效(期望如 1s、2s): %w", raw, err)
	}
	if d <= 0 {
		return 0, fmt.Errorf("--interval %q 无效(必须 > 0)", raw)
	}
	return d, nil
}
