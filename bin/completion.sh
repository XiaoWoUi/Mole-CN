...
emit_fish_completions() {
    ...
    printf '\n'
    printf 'complete -f -c %s -n "__fish_seen_subcommand_from clean" -l dry-run -s n -d "预览清理效果而不做任何更改"\n' "$cmd"
    printf 'complete -c %s -n "__fish_seen_subcommand_from clean" -l external -r -a "(__fish_complete_directories)" -d "清理外部卷上的 OS 元数据"\n' "$cmd"
    ...
    printf 'complete -f -c %s -n "__fish_seen_subcommand_from history" -l limit -r -d "限制最近条目数量"\n' "$cmd"
...
if [[ $# -eq 0 ]]; then
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 试运行模式${NC},不会修改 shell 配置文件"
        echo ""
    fi
    ...
    if [[ "$current_shell" == "fish" ]]; then
        ...
        if [[ -z "$completion_name" ]]; then
            ...
            if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
                remove_stale_completion_entries "$config_fish" "已从 config.fish 移除过期的补全条目" || true
            fi
        ...
        if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
            echo -e "${GRAY}${ICON_REVIEW} [试运行] 将写入 Fish 补全到:${NC}"
            echo "  $mole_file"
            echo "  $mo_file"
            echo ""
            echo -e "${GREEN}${ICON_SUCCESS}${NC} 试运行完成,未做任何更改"
            exit 0
        fi

        # Remove stale config.fish source-based entries (previous install method)
        if remove_stale_completion_entries "$config_fish" "已从 config.fish 移除过期的基于 source 的条目"; then
            echo ""
        fi
    ...
    if [[ -z "$completion_name" ]]; then
        if [[ -f "$config_file" ]] && grep -Eq "(^# Mole shell completion$|(mole|mo)[[:space:]]+completion)" "$config_file" 2> /dev/null; then
            if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
                echo -e "${GRAY}${ICON_REVIEW} [试运行] 将从 $config_file 移除过期的补全条目${NC}"
                echo ""
            else
                original_mode=""
                original_mode="$(stat -f '%Mp%Lp' "$config_file" 2> /dev/null || true)"
                ...
                echo -e "${GREEN}${ICON_SUCCESS}${NC} 已从 $config_file 移除过期的补全条目"
                echo ""
            fi
        fi
        log_error "PATH 中未找到 mole,请先安装 Mole 再启用补全"
    ...
    if [[ -f "$config_file" ]] && grep -Eq "(mole|mo)[[:space:]]+completion" "$config_file" 2> /dev/null; then
        if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
            echo -e "${GRAY}${ICON_REVIEW} [试运行] 将规范化 $config_file 中的补全条目${NC}"
            echo ""
            exit 0
        fi
        ...
        echo ""
        echo -e "${GREEN}${ICON_SUCCESS}${NC} 已在 $config_file 中更新 shell 补全"
        echo ""
        exit 0
    fi
    ...
    echo ""
    echo -e "${GRAY}将添加至 ${config_file}:${NC}"
    echo "  $completion_line"
    echo ""
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        echo -e "${GREEN}${ICON_SUCCESS}${NC} 试运行完成,未做任何更改"
    ...
    echo -e "${GREEN}${ICON_SUCCESS}${NC} 补全已添加至 $config_file"
    echo ""
    echo ""
    echo -e "${GRAY}立即生效:${NC}"
...
EOF
    ...
    zsh)
        ...
        printf "        _describe '子命令' subcommands\n"
        printf '        return\n'
        printf '    fi\n'
        printf "    case \"\$words[2]\" in\n"
        printf '        clean)\n'
        printf '            _arguments \\\n'
        printf "                '--dry-run[预览清理效果而不做任何更改]' \\\\\n"
        printf "                '-n[预览清理效果而不做任何更改]' \\\\\n"
        printf "                '--external[清理外部卷上的 OS 元数据]:路径:_files -/' \\\\\n"
        ...
        printf '            ;;\n'
        printf '        completion)\n'
        printf "            _arguments '1:shell 类型:(bash zsh fish)'\n"
        printf '            ;;\n'
        printf '        *)\n'
        printf "            _describe '子命令' subcommands\n"
        printf '            ;;\n'
...
为 mole 和 mo 命令设置 shell 补全。

自动安装:
  mole completion              # 自动检测 shell 并安装
  mole completion --dry-run    # 预览配置更改而不写入文件
