...
show_optimization_summary() {
    ...
    [[ $unchanged -gt 0 ]] && outcome_parts+=("$unchanged 未变化")
    [[ $skipped -gt 0 ]] && outcome_parts+=("$skipped 已跳过")
    [[ $unavailable -gt 0 ]] && outcome_parts+=("$unavailable 不可用")
    [[ $attention -gt 0 ]] && outcome_parts+=("$attention 需要关注")
    [[ $failed -gt 0 ]] && outcome_parts+=("$failed 失败")
    ...
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        ...
        if [[ $attention -gt 0 || $failed -gt 0 ]]; then
            summary_details+=("请查看上面的警告")
        else
            summary_details+=("优化流程已完成")
        fi
    fi
...
show_system_health() {
    ...
    printf "${ICON_ADMIN} 系统  %s/%s GB 内存 | %s/%s GB 磁盘 | 已运行 %sd\n" \
...
main() {
    ...
    # Dry-run indicator.
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 试运行模式${NC},不会修改任何文件\n"
    fi
    ...
    if ! json_validate "$health_json"; then
        ...
        echo -e "${GRAY}${ICON_REVIEW}${NC} 请检查 awk、sysctl、df 命令是否可用"
        exit 1
    fi
    ...
    if [[ ${#CURRENT_WHITELIST_PATTERNS[@]} -gt 0 ]]; then
        ...
        if [[ $count -le 3 ]]; then
            ...
            echo -e "${ICON_ADMIN} 活动白名单: ${patterns_list}"
        fi
    fi
