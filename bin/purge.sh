...
perform_purge() {
    ...
    if [[ -t 1 ]]; then
        ...
        echo -e "${PURPLE_BOLD}项目构建产物清理${NC}"
    fi
    ...
    local summary_heading="构建产物清理完成"
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
            summary_line="将释放: ${GREEN}${freed_size_human}${NC}"
        fi
        [[ $total_items_cleaned -gt 0 ]] && summary_line+=" | 项: $total_items_cleaned"
...
show_help() {
    ...
    echo "  --dry-run       预演清理操作,不做任何更改"
    echo "  --include-empty 显示大小为零的项目构建产物目录"
    echo "  --debug         启用调试日志"
    echo "  --help          显示此帮助信息"
    echo ""
...
main() {
    ...
    start_purge
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 预演模式${NC},不会移除任何项目构建产物"
        printf '\n'
    fi
    hide_cursor
    perform_purge
