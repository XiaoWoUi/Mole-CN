...
show_status() {
    if is_touchid_configured; then
        echo -e "${GREEN}${ICON_SUCCESS}${NC} 已为 sudo 启用 Touch ID"
    else
        echo -e "${YELLOW}☻${NC} 尚未为 sudo 配置 Touch ID"
    fi
}
...
enable_touchid() {
    ...
    if touchid_dry_run_enabled; then
        if is_touchid_configured; then
            ...
            echo -e "${GREEN}${ICON_SUCCESS} [试运行] 将为 sudo 启用 Touch ID${NC}"
        ...
        if [[ -f "$PAM_SUDO_LOCAL_FILE" ]] && grep -q "pam_tid.so" "$PAM_SUDO_LOCAL_FILE"; then
            ...
            if grep -q "pam_tid.so" "$PAM_SUDO_FILE"; then
                ...
                if secure_install_pam "$temp_file" "$PAM_SUDO_FILE" 2> /dev/null; then
                    echo -e "${GREEN}${ICON_SUCCESS} 已清理遗留配置${NC}"
                fi
            fi
...
disable_touchid() {
    ...
    if touchid_dry_run_enabled; then
        if ! is_touchid_configured; then
            ...
            echo -e "${GREEN}${ICON_SUCCESS} [试运行] 将为 sudo 禁用 Touch ID${NC}"
            echo -e "${GRAY}${ICON_REVIEW} 目标文件: ${PAM_SUDO_FILE} 和/或 ${PAM_SUDO_LOCAL_FILE}${NC}"
        fi
        return 0
    fi
...
show_menu() {
    ...
    show_status
    if is_touchid_configured; then
        echo -ne "${PURPLE}☛${NC} 按 ${GREEN}回车${NC} 禁用,${GRAY}Q${NC} 退出: "
...
main() {
    ...
    if touchid_dry_run_enabled; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 试运行模式${NC},不会修改任何 sudo 认证文件"
        echo ""
    fi
