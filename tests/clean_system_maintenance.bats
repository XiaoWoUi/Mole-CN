...
EOF2
    ...
    [ "$status" -eq 0 ]
    [[ "$output" != *"SUCCESS:第三方系统日志"* ]] || return 1
    [[ "$output" == *"safe_sudo_find_delete:/Library/Logs/Adobe:*"* ]] || return 1
    [[ "$output" == *"safe_sudo_find_delete:/Library/Logs/CreativeCloud:*"* ]] || return 1
...
EOF
    ...
    [[ "$output" == *"Homebrew autoremove 将移除以下内容"* ]] || return 1
    [[ "$output" == *"python@3.14"* ]] || return 1
    [[ "$output" == *"Homebrew autoremove · 已跳过"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Homebrew 链接 · 已恢复 1 个活动的可执行链接"* ]] || {
        echo "$output"
        return 1
    }
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Homebrew · 将执行清理"* ]] || return 1
    [[ "$output" == *"Homebrew autoremove 将移除以下内容"* ]] || return 1
    [[ "$output" == *"python@3.14"* ]] || return 1
    [[ "$output" == *"CALL:brew autoremove --dry-run"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"[预演] 将移除"* ]] || return 1
    [[ "$output" == *"1 个旧的系统内存异常报告"* ]]
}
...
EOF
    ...
    [[ "$output" == *"SUCCESS:可重建的 GPU 缓存,共 3 项"* ]]
}
...
EOF
    ...
    [[ "$output" == *"SUCCESS:可重建的 GPU 缓存,共 1 项"* ]] || return 1
}
