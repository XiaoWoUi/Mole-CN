...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"将清除早于 30 天的项(2MB)"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Youdao Dictionary 缓存"* ]] || return 1
    [[ "$output" == *"Eudict 缓存"* ]]
}
