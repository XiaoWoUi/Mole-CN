...
EOF
	...
	[[ "$status" -eq 0 ]] || { echo "$output"; return 1; }
	[[ "$output" == *"已跳过 1 个超过 100 MB 安全上限的数据库"* ]] || return 1
}
...
EOF
	...
	[[ "$status" -eq 0 ]] || { echo "$output"; return 1; }
	[[ "$output" == *"已启用 .DS_Store 预防"* ]] || return 1
	[[ "$output" == *"无法为 1 类卷启用 .DS_Store 预防"* ]] || return 1
}
...
EOF
	...
	[[ "$output" == *"未发现遗留的 App Nap 或磁盘映像覆盖项"* ]] || return 1
...
EOF
	...
	[[ "$output" == *"已移除覆盖项: App Nap 已在全局禁用 (NSAppSleepDisabled)"* ]] || return 1
	[[ "$output" == *"已移除覆盖项: 磁盘映像验证被跳过 (skip-verify)"* ]] || return 1
}
...
EOF
	...
	[ "$status" -eq 0 ] || return 1
	[[ "$output" == *"将移除覆盖项: App Nap 已在全局禁用 (NSAppSleepDisabled)"* ]] || return 1
	[[ "$output" != *"DELETE_CALLED"* ]] || return 1
}
...
EOF
	...
	[ "$status" -eq 0 ] || return 1
	[[ "$output" == *"已跳过(白名单): App Nap 已在全局禁用 (NSAppSleepDisabled)"* ]] || return 1
	[[ "$output" != *"DELETE_CALLED"* ]] || return 1
}
...
if printf '%s\n' "$json" | grep -q '持久化 Finder 偏好'; then
    echo "described"
fi
EOF
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"已清理 1 个损坏的 Launch Agent"* ]]
}
...
EOF
	...
	[[ "$status" -eq 0 ]] || { echo "$output"; return 1; }
	[[ "$output" == *"无法运行定期维护(退出码=7)"* ]] || return 1
}
...
EOF
	...
	[[ "$output" == *"仅挂载了系统托管的 CoreSimulator 映像,仅供参考,不是分离目标"* ]] || return 1
	[[ "$output" != *"评估开销"* ]]
}
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"未检测到持续的高 CPU 瓶颈"* ]] || return 1
	[[ "$output" != *"评估开销"* ]] || return 1
	[[ "$output" != *"将建议分离"* ]] || return 1
...
EOF
	...
	[[ "$single" == *"已分离 /Volumes/A"* ]] || return 1
	[[ "$single" != *"已挂载映像"* ]] || return 1
	[[ "$double" == *"已分离 2 个已挂载映像"* ]] || return 1
}
...
EOF
	...
	[ "$status" -eq 0 ] || { echo "$output"; return 1; }
	[[ "$output" == *'已分离 /Volumes/Success\033[2J-'* ]] || return 1
	[[ "$output" == *'分离失败: /Volumes/Failed\033[H-'* ]] || return 1
	[[ "$output" != *$'\033[2J'* ]] || return 1
	[[ "$output" != *$'\033[H'* ]]
}
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"已跳过(白名单): Finder 缓存刷新"* ]] || return 1
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"6/8 GB 内存"* ]] || return 1
	[[ "$output" == *"288/351 GB 磁盘"* ]] || return 1
	[[ "$output" == *"已运行 6d"* ]]
}
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"权限修复|disk_permissions_repair|optimize_task"* ]] || return 1
	[[ "$output" == *"登录项审计|login_items_audit|optimize_task"* ]] || return 1
	[[ "$output" == *"遗留覆盖项|legacy_overrides_audit|optimize_task"* ]] || return 1
}
