...
grep -q "ENSURE_SUDO:Homebrew casks 需要管理员权限: BrewApp" "$HOME/brew_calls.log"
...
grep -q "ENSURE_SUDO:Homebrew casks 需要管理员权限: BrewPreAuth" "$HOME/order.log"
grep -q "BREW_CALL:uninstall --cask --zap brew-preauth-cask" "$HOME/order.log"
[[ "$(sed -n '1p' "$HOME/order.log")" == "ENSURE_SUDO:Homebrew casks 需要管理员权限: BrewPreAuth" ]]
EOF
