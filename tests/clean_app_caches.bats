...
safe_clean() {
    case "${!#}" in
        "Xcode 缓存" | "Xcode 构建产物") echo "UNEXPECTED_XCODE_CLEAN:${!#}" ;;
    esac
}
clean_xcode_tools
EOF
    ...
    [[ "$output" != *"Xcode 缓存/构建产物 · 已跳过"* ]] || return 1
...
safe_clean() {
    case "${!#}" in
        "Autodesk 缓存") echo "UNEXPECTED_CLEAN:${!#}" ;;
    esac
}
...
EOF
    ...
    [[ "$output" == *"CLEAN:Final Cut Pro 生成的缓存"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" != *"Final Cut Pro 生成的缓存 · 已跳过"* ]] || return 1
...
EOF
    ...
    [[ "$output" == *"Final Cut Pro 生成的缓存 · 已跳过(进程状态未知)"* ]] || return 1
...
EOF
    ...
    [[ "$output" == *"CLEAN:JianyingPro 生成的缓存"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"JianyingPro 生成的缓存 · 已跳过(JianyingPro 正在运行)"* ]] || return 1
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"DingTalk iDingTalk 缓存"* ]] || return 1
    [[ "$output" == *"DingTalk 日志"* ]]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"QQ Music Mac 缓存"* ]] || return 1
    [[ "$output" == *"QQ Music 流媒体缓存"* ]] || return 1
    [[ "$output" == *"QQ Music 日志"* ]] || return 1
    [[ "$output" == *"QQ Music 容器缓存"* ]]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Tencent Video 旧版安装程序"* ]] || return 1
    [[ "$output" == *"Tencent Video 原生缓存"* ]] || return 1
    [[ "$output" == *"Tencent Video 文档缓存"* ]]
}
