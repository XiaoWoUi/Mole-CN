...
opt_cache_refresh() {
    ...
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        if [[ ${#removable_targets[@]} -eq 0 ]]; then
            debug_operation_detail "待移除文件" "无"
        else
            debug_operation_detail "待移除文件" ""
            local index
...
opt_saved_state_cleanup() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        debug_operation_start "App 保存状态清理" "移除旧的应用保存状态"
        ...
        debug_operation_detail "预期结果" "降低磁盘占用,应用以干净状态启动"
        debug_risk_level "低" "旧的保存状态,应用会重新创建"
    fi
    ...
    if [[ -d "$state_dir" ]]; then
        ...
        if ! scan_file=$(mktemp_file "optimize-saved-states"); then
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法准备保存状态扫描"
        ...
        if [[ $scan_rc -ne 0 ]]; then
            ...
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法扫描旧保存状态"
            scan_failed=1
        fi
    ...
    if [[ $scan_failed -eq 0 && $remove_failed -eq 0 ]]; then
        ...
        opt_msg "已移除 $removed 个旧保存状态"
    fi
    if [[ $remove_failed -gt 0 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法移除 $remove_failed 个旧保存状态"
    fi
...
opt_fix_broken_configs() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_risk_level "低" "应用下次启动时会重新生成偏好设置文件"
    fi
    ...
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        debug_operation_detail "已修复文件数" "$broken_prefs"
    fi

    export OPTIMIZE_CONFIGS_REPAIRED="${broken_prefs}"
    if [[ $broken_prefs -gt 0 ]]; then
        if [[ $prefs_partial -ne 0 ]]; then
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 偏好设置扫描已达时间预算,目前修复 ${broken_prefs:-0} 项"
        else
...
opt_network_optimization() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_risk_level "低" "DNS 缓存会自动重建"
    fi
...
opt_quarantine_cleanup() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        debug_operation_start "隔离区数据库清理" "清除 Gatekeeper 下载跟踪历史"
        debug_operation_detail "方法" "对 QuarantineEventsV2 SQLite 数据库执行 DELETE + VACUUM"
        debug_operation_detail "安全性" "仅清除下载跟踪元数据,不影响文件的隔离标记"
        debug_operation_detail "预期结果" "减小数据库体积,清除下载跟踪历史"
        debug_risk_level "低" "数据库由 macOS 自动重建"
    fi
    ...
    if [[ $count_status -ne 0 || ! "$row_count" =~ ^[0-9]+$ ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法检查隔离区数据库"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        return 0
    fi
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
        ...
        if [[ $exit_code -eq 0 ]]; then
            opt_msg "隔离区历史已清除(共 $row_count 条)"
            optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_APPLIED"
        else
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法清理隔离区数据库"
            optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        fi
    else
        opt_msg "隔离区历史已清除(共 $row_count 条)"
...
opt_sqlite_vacuum() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        debug_operation_start "数据库优化" "对 Mail、Safari、Messages 的 SQLite 数据库执行 VACUUM"
        debug_operation_detail "方法" "完整性检查后对数据库执行 VACUUM 命令"
        debug_operation_detail "安全检查" "应用运行时跳过,先验证完整性,20 秒超时"
        debug_operation_detail "预期结果" "减小数据库体积,提升应用性能"
        debug_risk_level "低" "仅优化数据库,不删除数据"
    fi
    ...
    for app in "${check_apps[@]}"; do
        if pgrep -x "$app" > /dev/null 2>&1; then
            ...
            if [[ $probe_status -ne 1 ]]; then
                echo -e "  ${YELLOW}${ICON_WARNING}${NC} 数据库优化前无法检查运行中的应用"
                optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
                return 0
            fi
        fi
    ...
    if ! command -v sqlite3 > /dev/null 2>&1; then
        echo -e "  ${GRAY}-${NC} 数据库优化不可用,sqlite3 不可用"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNAVAILABLE"
        return 0
    fi
    ...
    if [[ $vacuumed -gt 0 ]]; then
        opt_msg "已优化 $vacuumed 个数据库(Mail、Safari、Messages)"
    ...
    if [[ $already_optimal -gt 0 ]]; then
        opt_msg "$already_optimal 个数据库已是最优"
    fi

    if [[ $policy_skipped -gt 0 ]]; then
        opt_msg "已跳过 $policy_skipped 个超过 100 MB 安全上限的数据库"
...
opt_launch_services_rebuild() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "方法" "运行 lsregister -gc,再用 -r -f 强制重扫 local、user、system 域"
        debug_operation_detail "目的" "修复\"打开方式\"菜单问题、文件关联与过期应用元数据"
        debug_operation_detail "预期结果" "正确的应用关联,修复重复条目,减少过期应用列表"
        debug_risk_level "低" "数据库会自动重建"
    fi
...
opt_network_stack_optimize() {
    ...
    if [[ $route_status -gt 1 || $dns_status -gt 1 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法检查网络健康状态"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        return 0
    fi
...
opt_disk_permissions_repair() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "预期结果" "修复文件访问问题,所有权正确"
        debug_risk_level "中" "需要 sudo,会修改权限"
    fi
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
        ...
        if [[ "$success" == "true" ]]; then
            ...
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 权限修复失败,可能无需修复"
            optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        fi
    else
        opt_msg "用户目录权限已修复"
...
opt_spotlight_index_optimize() {
    ...
    if [[ $spotlight_status_code -ne 0 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法检查 Spotlight 索引(退出码=$spotlight_status_code)"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        return 0
    fi
...
opt_prevent_network_dsstore() {
    ...
    if [[ $changed -eq 0 && $already -gt 0 ]]; then
        opt_msg ".DS_Store 预防已启用(网络与 USB 卷)"
    fi

    if [[ $changed -gt 0 ]]; then
        opt_msg "已启用 .DS_Store 预防(网络与 USB 卷)"
    elif [[ $failed -gt 0 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法启用 .DS_Store 预防"
    fi
    if [[ $changed -gt 0 && $failed -gt 0 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法为 $failed 类卷启用 .DS_Store 预防"
    fi
...
opt_legacy_overrides_audit() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        debug_operation_start "遗留覆盖项" "检测 App Nap 与磁盘映像验证覆盖项"
        debug_operation_detail "方法" "defaults read -g NSAppSleepDisabled; defaults read com.apple.frameworks.diskimages skip-verify*"
        debug_operation_detail "预期结果" "移除覆盖项后 macOS 默认行为恢复生效"
        debug_risk_level "低" "仅删除显式覆盖键;macOS 恢复默认行为"
    fi
    ...
    if _opt_defaults_is_truthy "$value"; then
        found_labels+=("App Nap 已在全局禁用 (NSAppSleepDisabled)")
        found_domains+=("-g")
    ...
    if [[ ${#found_keys[@]} -eq 0 ]]; then
        opt_msg "未发现遗留的 App Nap 或磁盘映像覆盖项"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNCHANGED"
        return 0
    fi
    ...
    for idx in "${!found_keys[@]}"; do
        ...
        if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} 将移除覆盖项: ${found_labels[$idx]}"
            changed=$((changed + 1))
            continue
        fi
        if defaults delete "${found_domains[$idx]}" "${found_keys[$idx]}" 2> /dev/null; then
            opt_msg "已移除覆盖项: ${found_labels[$idx]}"
            changed=$((changed + 1))
        else
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法移除覆盖项: ${found_labels[$idx]}"
            failed=$((failed + 1))
        fi
    done
...
opt_launch_agents_cleanup() {
    ...
    if [[ $removed_count -gt 0 ]]; then
        opt_msg "已清理 $removed_count 个损坏的 Launch Agent"
    fi
    if [[ $failed -gt 0 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法移除 $failed 个损坏的 Launch Agent"
    fi
...
opt_periodic_maintenance() {
    ...
    if ! command -v periodic > /dev/null 2>&1; then
        opt_msg "定期维护已跳过(当前 macOS 版本不可用)"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNAVAILABLE"
        return 0
    fi
    ...
    if [[ -f "$daily_log" ]]; then
        ...
        if [[ $age_days -lt $stale_days ]]; then
            opt_msg "定期维护已是最新(${age_days} 天前运行)"
            optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNCHANGED"
            return 0
        fi
    fi
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
        ...
        if periodic_output=$(sudo periodic daily weekly monthly 2>&1); then
            ...
            rc=$?
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} 无法运行定期维护(退出码=$rc)"
            if [[ -n "$periodic_output" ]]; then
                debug_log "periodic 错误输出: $periodic_output"
            fi
            optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
        fi
    else
...
opt_notification_cleanup() {
    ...
    if ! nc_db=$(resolve_notification_center_db); then
        ...
        echo -e "  ${GRAY}-${NC} 通知中心数据库不可用(没有支持的路径)"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNAVAILABLE"
        return 0
    fi
    debug_log "通知中心数据库: $nc_db"
    ...
    if [[ $db_size -lt 51200 ]]; then
        opt_msg "通知中心数据库健康($(bytes_to_human $((db_size * 1024))))"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNCHANGED"
        return 0
    fi
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
        if command -v sqlite3 > /dev/null 2>&1; then
            ...
            if [[ $sql_ok -eq 0 ]]; then
                ...
                echo -e "  ${YELLOW}${ICON_WARNING}${NC} 通知中心清理已跳过(数据库繁忙或已锁定)"
                optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
            fi
        else
...
opt_disk_verify() {
    ...
    if [[ $verify_status -eq 124 ]]; then
        ...
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 磁盘验证失败(退出码=$verify_status)"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_FAILED"
...
opt_coreduet_cleanup() {
    ...
    if [[ $total_size -lt 102400 ]]; then
        opt_msg "Knowledge 数据库健康($(bytes_to_human $((total_size * 1024))))"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_UNCHANGED"
        return 0
    fi
    ...
    if [[ "${MOLE_DRY_RUN:-0}" != "1" ]]; then
        ...
        if [[ $sql_failed -gt 0 ]]; then
            echo -e "  ${YELLOW}${ICON_WARNING}${NC} Knowledge 数据库清理已跳过(数据库繁忙或已锁定)"
...
_login_item_bundle_metadata_matches() {
    ...
    for key in CFBundleDisplayName CFBundleName CFBundleExecutable; do
        ...
        if _login_item_name_matches "$value" "$name" "$nospace" "$stripped"; then
            _login_item_debug "'$name' 在 $app_path 匹配了 $key '$value'"
            return 0
        fi
    done
...
_login_item_app_exists() {
    ...
    if [[ -n "$item_path" ]]; then
        if [[ -e "$item_path" || -L "$item_path" ]]; then
            _login_item_debug "'$name' 通过登录项路径解析: $item_path"
            return 0
        fi
        _login_item_debug "'$name' 的登录项路径缺失: $item_path"
    else
        _login_item_debug "'$name' 没有来自 System Events 的登录项路径"
    fi

    # 1. Exact match
    if [[ "$name" != *"'"* ]] && mdfind "kMDItemFSName == '${name}.app'" 2> /dev/null | grep -q .; then
        _login_item_debug "'$name' 通过 Spotlight 精确应用名解析"
        return 0
    fi
    ...
    if [[ "$name" != *"'"* && "$nospace" != "$name" ]] && mdfind "kMDItemFSName == '${nospace}.app'" 2> /dev/null | grep -q .; then
        _login_item_debug "'$name' 通过 Spotlight 去空格应用名解析"
        return 0
    fi
    ...
    if [[ "$name" != *"'"* && "$stripped" != "$nospace" ]] && mdfind "kMDItemFSName == '${stripped}.app'" 2> /dev/null | grep -q .; then
        _login_item_debug "'$name' 通过 Spotlight 去除辅助后缀名解析"
        return 0
    fi
    ...
    for roots in "/Applications" "$HOME/Applications"; do
        ...
        if [[ -n "$candidate" && -d "$candidate" ]]; then
            _login_item_debug "'$name' 通过文件系统应用名解析: $candidate"
            return 0
        fi
    ...
    if [[ -n "$btm_path" ]] && [[ -e "$btm_path" ]]; then
        _login_item_debug "'$name' 通过 sfltool BTM 路径解析: $btm_path"
        return 0
    fi
    _login_item_debug "'$name' 经路径、Spotlight、文件系统与 BTM 检查后仍无法解析"
    return 1
}

opt_login_items_audit() {
    if [[ "${MOLE_TEST_NO_AUTH:-0}" == "1" ]]; then
        opt_msg "登录项审计·测试模式下跳过"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_SKIPPED"
        return 0
    fi
    ...
    if [[ $broken -eq 0 ]]; then
        ...
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} $broken 个损坏的登录项·请通过 System Settings > General > Login Items 移除"
        optimize_task_result "$MOLE_OPTIMIZE_OUTCOME_ATTENTION"
    fi
}
...
execute_optimization() {
    ...
    if ! handler=$(optimize_catalog_handler_for "$action"); then
        echo -e "${YELLOW}${ICON_ERROR}${NC} 未知动作: $action"
        return 1
    fi
    health_name=$(optimize_catalog_health_name_for "$action")
    if ! declare -F "$handler" > /dev/null; then
        echo -e "${YELLOW}${ICON_ERROR}${NC} 缺少优化处理器: $handler"
        return 1
    fi
