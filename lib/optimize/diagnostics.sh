...
opt_diag_family_note() {
    case "$1" in
        cloudshell)
            printf '%s\n' "检测到外部企业代理占用。Mole 不会终止企业安全进程;重启或策略检查必须在 Mole 之外进行。"
            ;;
        syspolicyd)
            printf '%s\n' "Gatekeeper 与代码签名评估活动偏高。"
            ;;
        windowserver)
            printf '%s\n' "桌面合成繁忙。若其他类别占比更高,这更可能是症状而非根因。"
            ;;
        spotlight)
            printf '%s\n' "元数据索引或导入任务正在消耗 CPU。"
            ;;
...
opt_diag_detach_candidates() {
    ...
    if [[ $detached -gt 1 ]]; then
        echo -e "  ${GRAY}${ICON_REVIEW}${NC} 已分离 ${detached} 个已挂载映像"
    fi
    if [[ $failed -gt 1 ]]; then
        echo -e "  ${GRAY}${ICON_REVIEW}${NC} ${failed} 个已挂载映像仍需手动处理"
    fi
}
...
opt_diag_offer_detach_candidates() {
    ...
    if [[ "$count" -eq 1 ]]; then
        echo -e "  ${GRAY}${ICON_LIST}${NC} 已挂载映像增加评估开销:"
    else
        echo -e "  ${GRAY}${ICON_LIST}${NC} 已挂载映像增加评估开销:"
    fi
    while IFS=$'\t' read -r image_path mount_path; do
    ...
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} 将建议分离 ${count} 个已挂载映像"
        return 0
    fi

    if [[ ! -t 1 ]]; then
        echo -e "  ${GRAY}${ICON_REVIEW}${NC} 请检查这些已挂载映像,分离不再需要的映像"
        return 0
    fi

    echo -ne "  ${GRAY}${ICON_REVIEW}${NC} ${YELLOW}立即分离?${NC} ${GRAY}回车确认 / 空格取消${NC}: "
    local key=""
    if ! key=$(read_key); then
        echo -e "\n  ${GRAY}${ICON_WARNING}${NC} 保持挂载,可通过 ${NC}mo optimize --whitelist${GRAY}${NC} 加入白名单"
        return 0
    fi
...
run_optimize_diagnostics() {
    ...
    if [[ -z "$primary_family" && "$extra_findings" -eq 0 ]]; then
        echo -e "  ${GREEN}${ICON_SUCCESS}${NC} 未检测到持续的高 CPU 瓶颈"
        ...
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} 疑似瓶颈: ${label} (~${primary_avg}% CPU 持续)"
        echo -e "  ${GRAY}${ICON_REVIEW}${NC} $(opt_diag_family_note "$primary_family")"

        if [[ $sustained_count -gt 1 ]]; then
            echo -e "  ${GRAY}${ICON_LIST}${NC} 其他持续占用:"
    ...
    if [[ "$primary_family" == "syspolicyd" || "$sustained_details" == *$'syspolicyd\t'* ]]; then
        ...
        if [[ "$managed_count" -gt 0 && "$managed_count" == "$coresim_count" && "$detach_count" -eq 0 ]]; then
            echo -e "  ${GRAY}${ICON_INFO}${NC} 仅挂载了系统托管的 CoreSimulator 映像,仅供参考,不是分离目标"
        fi
