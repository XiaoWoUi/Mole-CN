...
brew_uninstall_cask() {
    ...
    if [[ "${MOLE_DRY_RUN:-0}" == "1" ]]; then
        debug_log "[模拟运行] 将执行 brew ${uninstall_args[*]} $cask_name"
        return 0
    fi
    ...
    debug_log "正在尝试执行 brew ${uninstall_args[*]} $cask_name"
    ...
    if [[ -n "$app_path" && -d "$app_path" ]]; then
        ...
        debug_log "应用大小: ${size_gb}GB,超时时间: ${timeout}s"
    fi
    ...
    if [[ "$uninstall_ok" != "true" ]]; then
        debug_log "brew 卸载超时或失败,退出码: $brew_exit"
        ...
        if [[ $brew_exit -eq 124 ]]; then
            debug_log "brew 卸载在 ${timeout} 秒后超时,返回失败"
            return 124
    ...
    debug_log "brew 卸载失败: cask_gone=$cask_gone app_gone=$app_gone"
    return 1
}
