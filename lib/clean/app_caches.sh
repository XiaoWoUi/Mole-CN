...
clean_xcode_derived_data() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        ...
        if [[ -n "$dry_run_stopped_reason" ]]; then
            if [[ "$dry_run_stopped_reason" == "进程状态未知" ]]; then
    ...
    if [[ "$simulator_has_targets" == "true" ]]; then
        ...
        if [[ $simulator_state -eq 1 ]]; then
            ...
            if [[ $simulator_state -eq 2 ]]; then
                echo -e "  ${GRAY}${ICON_WARNING}${NC} Simulator 缓存 · 已跳过(进程状态未知)"
                note_activity
            else
...
clean_communication_apps() {
    ...
    safe_clean ~/Library/Caches/com.tencent.xinWeChat/* "WeChat 缓存"
    ...
    safe_clean ~/Library/Caches/com.tencent.meeting/* "Tencent Meeting 缓存"
    safe_clean ~/Library/Caches/com.tencent.WeWorkMac/* "WeCom 缓存"
    safe_clean ~/Library/Caches/com.tencent.qq/* "QQ 缓存"
    safe_clean ~/Library/Caches/com.feishu.*/* "Feishu 缓存"
...
# DingTalk.
clean_dingtalk() {
    safe_clean ~/Library/Caches/dd.work.exclusive4aliding/* "DingTalk iDingTalk 缓存"
    safe_clean ~/Library/Caches/com.alibaba.AliLang.osx/* "AliLang 安全组件"
    if [[ -d ~/Library/Application\ Support/iDingTalk ]]; then
        safe_clean ~/Library/Application\ Support/iDingTalk/log/* "DingTalk 日志"
        safe_clean ~/Library/Application\ Support/iDingTalk/holmeslogs/* "DingTalk holmes 日志"
    fi
}
# AI assistants.
clean_ai_apps() {
    safe_clean ~/Library/Caches/com.openai.chat/* "ChatGPT 缓存"
    safe_clean ~/Library/Caches/com.anthropic.claudefordesktop/* "Claude 桌面端缓存"
...
clean_final_cut_pro_generated_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        if [[ $process_state -eq 2 ]]; then
            echo -e "  ${GRAY}${ICON_WARNING}${NC} Final Cut Pro 生成的缓存 · 已跳过(进程状态未知)"
            note_activity
        else
...
clean_jianying_pro_generated_caches() {
    ...
    if [[ $process_state -ne 1 ]]; then
        local skip_reason="JianyingPro 正在运行"
        [[ $process_state -eq 2 ]] && skip_reason="进程状态未知"
        echo -e "  ${GRAY}${ICON_WARNING}${NC} JianyingPro 生成的缓存 · 已跳过($skip_reason)"
        note_activity
        return 0
    fi
    ...
    safe_clean "${targets[@]}" "JianyingPro 生成的缓存"
}
...
clean_productivity_apps() {
    safe_clean ~/Library/Caches/com.tw93.MiaoYan/* "MiaoYan 缓存"
    safe_clean ~/Library/Caches/com.klee.desktop/* "Klee 缓存"
    safe_clean ~/Library/Caches/klee_desktop/* "Klee 桌面端缓存"
    ...
    safe_clean ~/Library/Application\ Support/Quark/Cache/videoCache/* "Quark 视频缓存"
...
clean_media_players() {
    ...
    safe_clean ~/Library/Containers/com.apple.podcasts/Data/tmp/StreamedMedia "Podcasts 流媒体缓存"
    safe_clean ~/Library/Containers/com.apple.podcasts/Data/tmp/*.heic "Podcasts 封面缓存"
    safe_clean ~/Library/Containers/com.apple.podcasts/Data/tmp/*.img "Podcasts 图片缓存"
    safe_clean ~/Library/Containers/com.apple.podcasts/Data/tmp/*CFNetworkDownload*.tmp "Podcasts 下载临时文件"
    safe_clean ~/Library/Caches/com.apple.TV/* "Apple TV 缓存"
    safe_clean ~/Library/Caches/tv.plex.player.desktop "Plex 缓存"
    safe_clean ~/Library/Caches/com.netease.163music "NetEase Music 缓存"
    safe_clean ~/Library/Caches/com.tencent.QQMusic/* "QQ Music 缓存"
    safe_clean ~/Library/Caches/com.tencent.QQMusicMac/* "QQ Music Mac 缓存"
    ...
    if [[ -d "$_qqmusic_container" ]]; then
        safe_clean "$_qqmusic_container/iRRCache"/* "QQ Music 流媒体缓存"
        safe_clean "$_qqmusic_container/iLog"/* "QQ Music 日志"
        safe_clean "$_qqmusic_container/iCache"/* "QQ Music 缓存"
        safe_clean "$_qqmusic_container/iTemp"/* "QQ Music 临时文件"
    fi
    safe_clean ~/Library/Containers/com.tencent.QQMusicMac/Data/Library/Caches/* "QQ Music 容器缓存"
    safe_clean ~/Library/Caches/com.kugou.mac/* "Kugou Music 缓存"
    safe_clean ~/Library/Caches/com.kuwo.mac/* "Kuwo Music 缓存"
}
# Video players.
clean_video_players() {
    ...
    safe_clean ~/Library/Caches/com.iqiyi.player "iQIYI 缓存"
    safe_clean ~/Library/Caches/com.tencent.tenvideo "Tencent Video 缓存"
    ...
    if [[ -d "$_tenvideo_as" ]]; then
        safe_clean "$_tenvideo_as/Upgrade"/* "Tencent Video 旧版安装程序"
        safe_clean "$_tenvideo_as/VideoNative"/* "Tencent Video 原生缓存"
        safe_clean "$_tenvideo_as/documentCache"/* "Tencent Video 文档缓存"
    fi
    safe_clean ~/Library/Caches/tv.danmaku.bili/* "Bilibili 缓存"
    safe_clean ~/Library/Caches/com.douyu.*/* "Douyu 缓存"
    safe_clean ~/Library/Caches/com.huya.*/* "Huya 缓存"
...
clean_neatdm_stale_segments() {
    ...
    if [[ $stale_count -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} NeatDM 过期下载 · ${stale_count} 项, $(colorize_human_size "$size_human") ${YELLOW}预演${NC}"
        else
            local line_color
            line_color=$(cleanup_result_color_kb "$stale_kb")
            echo -e "  ${line_color}${ICON_SUCCESS}${NC} NeatDM 过期下载 · ${stale_count} 项, ${line_color}${size_human}${NC}"
        fi
...
clean_translation_apps() {
    safe_clean ~/Library/Caches/com.youdao.YoudaoDict "Youdao Dictionary 缓存"
    safe_clean ~/Library/Caches/com.eudic.* "Eudict 缓存"
...
clean_system_utils() {
    ...
    safe_clean ~/Library/Application\ Support/WeType/DictUpdate/* "WeType 词典更新缓存"
...
clean_note_apps() {
    ...
    safe_clean ~/Library/Caches/com.yinxiang.*/* "Yinxiang Note 缓存"
}
...
clean_remote_desktop() {
    ...
    safe_clean ~/Library/Caches/com.sunlogin.*/* "Sunlogin 缓存"
}
