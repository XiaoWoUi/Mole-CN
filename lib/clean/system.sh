...
show_large_active_powerlog_notice() {
    ...
    echo -e "  ${YELLOW}${ICON_WARNING}${NC} 电源遥测数据库 · ${GREEN}${size_human}${NC} · ${GRAY}使用中,已保留 · $(format_path_link "$MOLE_ACTIVE_POWERLOG_DB_PATH")${NC}"
}
...
report_system_cleanup_incomplete() {
    ...
    if [[ "$status" -eq 124 ]]; then
        echo -e "  ${YELLOW}${ICON_WARNING}${NC} ${label} · ${GRAY}已超时,清理可能不完整${NC}"
...
report_system_cleanup_budget_reached() {
    echo -e "  ${YELLOW}${ICON_WARNING}${NC} 系统清理 · ${GRAY}已达时间限制,已跳过剩余慢速扫描${NC}"
    if declare -F note_activity > /dev/null 2>&1; then
        note_activity
    fi
}
...
clean_deep_system() {
    ...
    start_section_spinner "正在扫描 macOS 安装器文件..."
    ...
    if [[ $installer_status -ne 0 ]]; then
        report_system_cleanup_incomplete "macOS 安装器文件" "$installer_status"
    fi
    if system_cleanup_budget_reached "$system_cleanup_deadline"; then
        report_system_cleanup_budget_reached
        return 0
    ...
    if [[ $code_sign_cleaned -gt 0 ]]; then
        log_success "浏览器代码签名缓存,共 $code_sign_cleaned 项"
    fi
    if system_cleanup_budget_reached "$system_cleanup_deadline"; then
        report_system_cleanup_budget_reached
        return 0
    ...
    if [[ $rebuildable_cache_cleaned -gt 0 ]]; then
        ...
        log_success "可重建的系统缓存,共 $rebuildable_cache_cleaned $rebuildable_cache_label"
    fi
    ...
    if [[ $gpu_scan_rc -ne 0 ]]; then
        ...
        report_system_cleanup_incomplete "可重建的 GPU 缓存" "$gpu_scan_rc"
    fi
    if [[ $gpu_cache_cleaned -gt 0 ]]; then
        ...
        log_success "可重建的 GPU 缓存,共 $gpu_cache_cleaned $gpu_cache_label"
    fi
    if system_cleanup_budget_reached "$system_cleanup_deadline"; then
        report_system_cleanup_budget_reached
        return 0
    ...
    if [[ $mem_rc -eq 0 && $mem_removed_count -gt 0 ]]; then
        if [[ "${DRY_RUN:-}" != "true" ]]; then
            ...
            log_info "[预演] 将移除 $mem_removed_count 个旧的系统内存异常报告($total_size_kb KB)"
        else
            log_info "[预演] 将移除 $mem_removed_count 个旧的系统内存异常报告"
        fi
    elif [[ $mem_rc -ne 0 ]]; then
        report_system_cleanup_incomplete "内存异常报告" "$mem_rc"
    elif [[ $stats_rc -eq 124 ]]; then
        report_system_cleanup_incomplete "内存异常报告大小统计" "$stats_rc"
    fi
    stop_section_spinner
        ...
        if [[ -d "$backupdb_dir" ]]; then
            ...
            while IFS= read -r -d '' inprogress_file; do
                ...
                if [[ "$DRY_RUN" == "true" ]]; then
                    ...
                    echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} 未完成的备份: $backup_name${NC} · $(colorize_human_size "$size_human") ${YELLOW}预演${NC}"
                ...
                if [[ $tm_delete_rc -eq 0 ]]; then
                    ...
                    echo -e "  ${YELLOW}!${NC} 无法删除: $backup_name · 请尝试手动使用 sudo"
            ...
            if [[ -n "$mounted_path" && -d "$mounted_path" ]]; then
                ...
                while IFS= read -r -d '' inprogress_file; do
                    ...
                    if [[ "$DRY_RUN" == "true" ]]; then
                        ...
                        echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} 未完成的 APFS 备份(位于 $bundle_name): $backup_name${NC} · $(colorize_human_size "$size_human") ${YELLOW}预演${NC}"
                        tm_cleaned=$((tm_cleaned + 1))
                        note_activity
                        continue
                    fi
                    ...
                    if [[ $tm_delete_rc -eq 0 ]]; then
                        ...
                        echo -e "  ${YELLOW}!${NC} 无法从 bundle 中删除: $backup_name"
