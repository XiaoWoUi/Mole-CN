...
check_tcc_permissions() {
    ...
    if [[ "$needs_permission_check" == "true" ]]; then
        ...
        echo -e "${GRAY}你可能会看到 ${GREEN}${#tcc_dirs[@]} 个权限对话框${NC}${GRAY},请全部允许。${NC}"
        echo ""
    ...
    if [[ $cleaned_size -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" != "true" ]]; then
            if [[ $protected_count -gt 0 ]]; then
                echo -e "  ${line_color}${ICON_SUCCESS}${NC} $browser_name Service Worker${NC} · ${line_color}${cleaned_human}${NC}, ${protected_count} 个已保护"
            else
                echo -e "  ${line_color}${ICON_SUCCESS}${NC} $browser_name Service Worker${NC} · ${line_color}${cleaned_human}${NC}"
            fi
        else
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} $browser_name Service Worker,将清理 $(colorize_human_size "$cleaned_human"),${protected_count} 个已保护"
        fi
        note_activity
...
clean_python_bytecode_cache_group() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        if [[ $skipped_count -gt 0 ]]; then
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Python 字节码缓存 · ${display_root}${NC} · ${YELLOW}${removed_count} 个目录, $(colorize_human_size "$size_human") ${YELLOW}预演, ${skipped_count} 个已跳过${NC}"
        else
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Python 字节码缓存 · ${display_root}${NC} · ${YELLOW}${removed_count} 个目录, $(colorize_human_size "$size_human") ${YELLOW}预演${NC}"
        fi
    else
        local line_color
...
clean_project_caches() {
    ...
    if [[ $failed_scan_count -gt 0 ]]; then
        echo -e "  ${GRAY}${ICON_WARNING}${NC} 项目缓存 · 跳过 ${failed_scan_count} 个缓慢/未完成的根目录扫描"
        if declare -f note_activity > /dev/null 2>&1; then
            note_activity
        fi
    fi
