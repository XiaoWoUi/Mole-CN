...
show_brew_autoremove_preview() {
    local preview_file="$1"
    echo -e "  ${GRAY}${ICON_WARNING}${NC} Homebrew autoremove 将移除以下内容:"
    sed 's/^/    /' "$preview_file"
}
...
restore_homebrew_active_links() {
    ...
    if [[ $restored -gt 0 ]]; then
        echo -e "  ${GREEN}${ICON_SUCCESS}${NC} Homebrew 链接 · 已恢复 ${restored} 个活动的可执行链接"
        note_activity
    fi
    if [[ $failed -gt 0 ]]; then
...
clean_homebrew() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        ...
        if is_path_whitelisted "$HOME/Library/Caches/Homebrew"; then
            ...
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Homebrew · 将执行清理"
            note_activity
            ...
            if [[ $dry_run_autoremove_exit -eq 0 ]] && brew_autoremove_preview_has_items "$dry_run_autoremove_file"; then
                ...
                echo -e "  ${GRAY}${ICON_WARNING}${NC} Autoremove 预演超时 · 请手动运行 ${GRAY}brew autoremove --dry-run${NC}"
            fi
        fi
        return 0
    fi
    ...
    if [[ "$skip_cleanup" == "false" ]]; then
        ...
        if [[ -t 1 ]]; then MOLE_SPINNER_PREFIX="  " start_inline_spinner "正在执行 Homebrew 清理..."; fi
    ...
    if [[ "$skip_cleanup" == "true" ]]; then
        ...
        if [[ $removed_count -gt 0 ]] || [[ -n "$freed_space" ]]; then
            if [[ -n "$freed_space" ]]; then
                ...
                echo -e "  ${GREEN}${ICON_SUCCESS}${NC} Homebrew 清理 · ${removed_count} 项"
                note_activity
            fi
        fi
    ...
    if [[ $autoremove_preview_exit -eq 124 ]]; then
        ...
        echo -e "  ${GRAY}${ICON_WARNING}${NC} Autoremove 预演失败 · 请手动运行 ${GRAY}brew autoremove --dry-run${NC}"
        note_activity
    elif brew_autoremove_preview_has_items "$autoremove_preview_file"; then
        show_brew_autoremove_preview "$autoremove_preview_file"
        echo -e "  ${GRAY}${ICON_WARNING}${NC} Homebrew autoremove · 已跳过(请手动运行 ${GRAY}brew autoremove${NC})"
        note_activity
    fi
