...
remove_mole() {
    ...
    if [[ "$dry_run_mode" == "true" ]]; then
        echo -e "${YELLOW}${ICON_DRY_RUN} 试运行模式${NC},不会移除任何文件"
        echo ""
        echo -e "${YELLOW}移除 Mole${NC},将删除以下内容:"
        if [[ "$is_homebrew" == "true" ]]; then
            echo -e "  ${GRAY}${ICON_LIST} 将运行: brew uninstall --force mole${NC}"
        fi
        if [[ ${manual_count:-0} -gt 0 ]]; then
            for install in "${manual_installs[@]}"; do
                [[ -f "$install" ]] && echo -e "  ${GRAY}${ICON_LIST} 将移除: ${install}${NC}"
            done
        fi
        if [[ ${alias_count:-0} -gt 0 ]]; then
            for alias in "${alias_installs[@]}"; do
                [[ -f "$alias" ]] && echo -e "  ${GRAY}${ICON_LIST} 将移除: ${alias}${NC}"
            done
        fi
        [[ -d "$HOME/.cache/mole" ]] && echo -e "  ${GRAY}${ICON_LIST} 将移除: $HOME/.cache/mole${NC}"
        [[ -d "$HOME/.config/mole" ]] && echo -e "  ${GRAY}${ICON_LIST} 将移至废纸篓: $HOME/.config/mole${NC}"
        [[ -d "$HOME/Library/Logs/mole" ]] && echo -e "  ${GRAY}${ICON_LIST} 将移除: $HOME/Library/Logs/mole${NC}"

        printf '\n%s\n\n' "${GREEN}${ICON_SUCCESS}${NC} 试运行完成,未做任何更改"
        exit 0
    fi
    ...
    if [[ "$is_homebrew" == "true" ]]; then
        if [[ -z "$brew_cmd" ]]; then
            log_error "未找到 Homebrew 命令。请确保已安装 Homebrew 并位于 PATH 中。"
    ...
    if [[ -d "$HOME/.config/mole" ]]; then
        ...
        if ! mkdir -p "$HOME/.Trash" 2> /dev/null ||
            ...
            has_error=true
            log_warning "无法将 ~/.config/mole 移至废纸篓;已保留原位置"
        fi
    fi
    ...
    if [[ "$has_error" == "true" ]]; then
        final_message="${YELLOW}${ICON_ERROR} Mole 卸载时出现部分错误,感谢使用 Mole!${NC}"
    else
